...
// Personal events used to have a single `time` field; migrate any events
// saved under that older shape to `startTime` so existing data keeps working.
function migrateEvents(events) {
  return events.map((e) => {
    if (e.time !== undefined && e.startTime === undefined) {
      const { time, ...rest } = e;
      return { ...rest, startTime: time, endTime: e.endTime ?? null };
    }
    return e;
  });
}
...
export function StoreProvider({ children }) {
  ...
  const importData = useCallback((parsed) => {
    ...
    setData({
      ...
      events: migrateEvents(incoming.events),
    });
    return { success: true };
  }, []);
