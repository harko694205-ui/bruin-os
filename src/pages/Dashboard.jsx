...
import {
  todayStr, formatFriendlyDate, formatTime, MONTH_NAMES, DAY_NAMES,
...
import {
  Sparkles, Timer, CalendarClock, AlertCircle, Link2, Settings2, ChevronRight,
} from 'lucide-react';
...
export default function Dashboard({ goTo, onStartFocus }) {
  ...
  const todaysClasses = useMemo(
    () => classes.filter((c) => c.days?.includes(dayIdx)).sort((a, b) => (a.startTime || '').localeCompare(b.startTime || '')),
    [classes, dayIdx]
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card card-pad">
          ...
          {todaysClasses.length === 0 ? (
            <p style={{ fontSize: 13.5, color: 'var(--text-tertiary)' }}>No classes scheduled today.</p>
          ) : (
                ...
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>
                      {c.startTime ? `${formatTime(c.startTime)} – ${formatTime(c.endTime)}` : 'No time set'}{c.room ? ` · ${c.room}` : ''}
                    </div>
                  </div>
                </div>
              ))}
