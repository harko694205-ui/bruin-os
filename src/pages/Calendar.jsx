...
import {
  ...
  getQuarterWeeks, MONTH_NAMES, DAY_ABBR, formatFriendlyDate, formatTimeRange,
} from '../lib/dates';
          ...
          {week.map((dateStr) => {
            ...
            return (
              <div
                ...
                style={{
                  minHeight: 92, minWidth: 0, padding: '8px 6px', borderRight: '1px solid var(--border)',
...
function WeekView({ weekStart, data, onSelectItem, onAddForDate }) {
  ...
  return (
    <>
      <div className="week-view-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: 10 }}>
        {dates.map((dateStr) => {
          const d = parseDateStr(dateStr);
          const items = getItemsForDate(data, dateStr);
          const isToday = dateStr === today;
          return (
            <div key={dateStr} className="card card-pad" style={{ borderColor: isToday ? 'var(--accent)' : 'var(--border)', minHeight: 200, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, minWidth: 0 }}>
                ...
                {items.map((item) => (
                  ...
                  >
                    {item.time && <span style={{ opacity: 0.7 }}>{formatTimeRange(item.time, item.endTime)} · </span>}
                    {item.title}
                  </button>
                ))}
              </div>
      ...
      <style>{`
        @media (max-width: 900px) {
          .week-view-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </>
  );
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              ...
              >
                ...
                {item.time && <span style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>{formatTimeRange(item.time, item.endTime)}</span>}
